...
var damage: float = 0.0 # The amount of damage dealt after 11.0s after firing
	...
	if fire:
		...
		if thingy > 0.02:
			for i in range(int(thingy / 0.02)):
				...
				projectile.linear_velocity = Vector2.from_angle(rotation + randf_range(-0.03, 0.03)) * 1536
